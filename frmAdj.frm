VERSION 5.00
Begin VB.Form frmAdj 
   BackColor       =   &H0080FFFF&
   Caption         =   "Tính tu trái nghia"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   4680
   ...
   Begin VB.ListBox lstResults 
      ...
      EndProperty
      Height          =   8040
      ItemData        =   "frmAdj.frx":0442
      Left            =   120
      List            =   "frmAdj.frx":0444
      TabIndex        =   10
   ...
   Begin VB.Menu mnuTest 
      ...
      Begin VB.Menu mnuTestEV 
         Caption         =   "T&iêng Anh sang tiêng Viêt"
         Shortcut        =   ^E
      End
...
Sub over(nd As String, title As String)
    ...
    lblKey.Caption = ""
End Sub
Sub hv(s1 As String, s2 As String)
    Dim p As String
    p = s1
    s1 = s2
    s2 = p
...
Private Sub imgEnter_Click()
    ...
    Loop
    If (txtAns <> "") And (LCase(txtAns) = b(tt).e4) Then
        With lblResult
            .ForeColor = &HFF0000
            .Caption = "Ñuùng"
        End With
    ...
    Else
        With lblResult
            ...
            .Caption = "Sai"
        End With
        lblKey.Caption = b(tt).e4
        f = f + 1
        With lblIncorrect
            .Caption = Mid(.Caption, 1, 5) & f
        End With
        With b(tt)
            lstResults.AddItem .e & "     " & .e4
        End With
    End If
    If m_ < m Then
        inc
        lblWord.Caption = b(tt).e
        txtAns = ""
    Else
...
Private Sub tmrTime_Timer()
    If (min = 0) And (sec = 0) Then
        ...
        For i = 0 To m - 1
            If b(i).chosen Then
                lstResults.AddItem b(i).e & "     " & b(i).e4
            End If
        Next i
        Exit Sub
    End If
...
Private Sub txtAns_Click()
    ...
    m = Val(txtAmount)
    If m > limit Then
        MsgBox "Sô luong tu không duoc vuot qua " & limit & "!", vbExclamation, "Sô luong tu không hop lê!"
        Exit Sub
    End If
    ...
    For i = 1 To m
        ...
        Do
            j = Int(Rnd * n)
        Loop Until (a(j).chosen = False) And (a(j).e4 <> "")
        b(i - 1) = a(j)
        b(i - 1).chosen = True
        a(j).chosen = True
    Next i
